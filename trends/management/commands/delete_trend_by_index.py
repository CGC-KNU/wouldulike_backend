...
class Command(BaseCommand):
    help = (
        "트렌드 리스트에서 특정 순서(인덱스)의 트렌드를 삭제합니다. "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "index",
            type=int,
            help="삭제할 트렌드의 순서 (1부터 시작, 최신이 1번)",
        )
        parser.add_argument(
            "--force",
            action="store_true",
            help="추가 확인 없이 바로 삭제합니다.",
        )
    ...
    def handle(self, *args, **options):
        ...
        # 비대화형 환경을 고려해, --force 없이도 기본적으로 삭제하되,
        # 어떤 데이터가 삭제되는지 로그를 충분히 남깁니다.
        self.stdout.write(self.style.WARNING(f"[{index}번째 트렌드를 삭제합니다]"))
        ...
        if not force:
            self.stdout.write(
                "주의: 이 명령어는 비대화형 환경에서 실행될 수 있으므로, "
                "--force 없이 실행해도 바로 삭제됩니다. 되돌릴 수 없습니다."
            )

        trend.delete()

        self.stdout.write(
            self.style.SUCCESS(f"[{index}번째 트렌드(id={trend.id})가 삭제되었습니다]")
        )
