...
class Command(BaseCommand):
    help = "현재 존재하는 모든 카카오 ID(User)의 FCM 토큰을 모아 한 번에 알림을 전송합니다."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"대상 사용자 수: {total_users}명, 유효한 고유 FCM 토큰 수: {len(tokens)}개"
            )
        )
        ...
        if not tokens:
            self.stdout.write(
                self.style.ERROR(
                    "전송할 수 있는 FCM 토큰이 없습니다. User.fcm_token 값을 확인하세요."
                )
            )
            return
